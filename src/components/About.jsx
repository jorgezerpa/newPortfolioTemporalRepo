  ...
  return (
    <>
      <section className="w-full flex flex-col md:flex-row min-h-[70vh] mt-14 py-14">
          ...
          <div className='w-full mt-10 md:mt-0 pl-5 md:pl-0'>
            <h2 ref={myJournyTitleRef} className={`opacity-0 ${inViewmyJournyTitle && 'animate-slideFR'} text-4xl sm:text-5xl font-bold text-left `}>My Journy</h2>
              ...
              { journyCards.map((item, key)=>(
                <div key={'journyItem'+key} className={`${inViewmyJournyItems && 'animate-scaleY2'} opacity-0 origin-top mt-4`}>
                  ...
                  { item.hilights.map((text, i)=>(
                    <div className='pl-4'>
                      <p key={text+i}>
                        <span className='inline-block w-1 h-1 rounded-lg bg-black relative -translate-y-1'></span>
                        {text}
                      </p>
                    </div>
                  ))}
      ...
      <section className="w-full mt-10">
          <h2 className="text-gray-900 font-bold text-2xl lg:text-3xl mb-10 text-center">Skills & technologies</h2>
          <div className='flex w-full justify-center'>
            <ul ref={refSkills} className={`max-w-[800px] flex justify-center gap-4 flex-wrap`} id="skillsGrid">
              {skills.map((skill, index)=>(
                  <li key={'skills'+skill+index} className={`${inViewSkills && `animate-scaleY`} origin-bottom opacity-0 rounded-md w-[120px] h-[50px] bg-blue-700 text-white border-lg shadow-sm shadow-gray-800`}>
